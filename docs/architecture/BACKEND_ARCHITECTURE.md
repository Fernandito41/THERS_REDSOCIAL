...
- **Migraciones — v0.13:** `backend/migrations/` (Flask-Migrate/Alembic), con cuatro migraciones: `a1b2c3d4e5f6_create_users_table` (inicial, `email` único), `a1edcbff74d8_add_profile_fields_to_users` (v0.7, `ADR-002`), `b2f4a19c3d7e_add_username_changed_at_to_users` (v0.9, `ADR-003`), y `f3a8c1d9e274_create_posts_table` (nueva, v0.13, `ADR-004`) — crea `posts` (FK `author_id → users.id`, `ON DELETE CASCADE`; índice `ix_posts_created_at`, justificado por `GET /api/posts` ordenando por fecha; trigger `trg_posts_updated_at`, reutiliza la función `set_updated_at()` ya creada por la migración inicial, sin duplicarla). Las cuatro verificadas con `flask db upgrade`/`downgrade` — la última, con ejecución real en esta tarea (`alembic_version` en `thers_test` confirmado en `f3a8c1d9e274`, cabeza de la cadena, tras un ciclo completo de downgrade a `b2f4a19c3d7e` y upgrade de vuelta).
- **`SQLAlchemyPostRepository` — nuevo, v0.13:** `infrastructure/persistence/repositories/post_repository.py`, implementa el puerto `PostRepository` (`domain/posts/repositories.py`, ver §7). `create()` inserta un `Post`, hace `commit()` + `refresh()` para traer `id`/`created_at` generados por PostgreSQL, y resuelve `post.author` antes de que la sesión se cierre. `list_recent(limit)` hace `SELECT ... ORDER BY created_at DESC LIMIT :limit`. El modelo `Post` (`infrastructure/persistence/models.py`) usa `author = db.relationship("User", lazy="joined")` para resolver el autor en el mismo `SELECT` que la lista de posts, evitando N+1.
...
- **Estructura — v0.13:** `backend/tests/` (`conftest.py`, `test_auth.py`, `test_users_me.py`, `test_update_profile.py`, `test_username_policy.py`, `test_error_handlers.py`, `test_config_jwt_secret.py` y `test_posts.py` — estos dos últimos nuevos, v0.12/v0.13 respectivamente). **75 pruebas** (59 de v0.11 + 4 de v0.12 `test_config_jwt_secret.py` + 12 de v0.13 `test_posts.py`), **ejecutadas y verificadas en esta tarea** (`pytest -v`, 75 passed, 0 failed, 0 skipped). `test_config_jwt_secret.py` (v0.12) corre `Config` en subprocesos aislados con `subprocess.run`, porque el cuerpo de la clase solo se evalúa una vez por proceso: sin `JWT_SECRET_KEY` ni opt-in falla al importar; con el opt-in usa el fallback; el opt-in con un valor distinto de `"1"` sigue fallando; un `JWT_SECRET_KEY` real nunca necesita el opt-in. `test_posts.py` (v0.13) cubre: creación (autor público correcto, sin token → 401, body vacío/contenido vacío/exceso de 2000 caracteres → 400, contenido de exactamente 2000 caracteres aceptado, `trim()` de espacios, campos no whitelisted ignorados — `author_id`/`id` del body nunca se usan) y listado (sin token → 401, orden por fecha descendente, posts de todos los autores visibles — feed global, lista vacía si no hay posts). `conftest.py` actualizado (v0.13): la fixture de limpieza (renombrada `_clean_tables`, antes `_clean_users_table`) ahora trunca `posts` junto con `users` en la misma sentencia — con la FK nueva, truncar solo `users` falla si hay posts dependientes. `test_error_handlers.py` (v0.10) cubre: `404` en una URL sin ruta, `405` en un verbo no permitido, `400` ante un body no-JSON, y `500` ante una excepción forzada con `monkeypatch` — verificando que la respuesta nunca exponga traceback ni datos sensibles. `test_auth.py` cubre duplicidad de `username`, formato de `username`/`phone`/`country_code`/`birth_date`/`email`/`password`, edad mínima, y `confirm_password` no coincidente. `test_users_me.py` cubre token válido/ausente/inválido/expirado, usuario inexistente, forma de la respuesta pública. `test_update_profile.py` cubre actualización parcial, whitelist, `phone`/`country_code` como par atómico, cooldown de `username`. `test_username_policy.py` cubre `can_change_username()` de forma puramente unitaria.
...
│   │       └── post_routes.py        # IMPLEMENTADO — v0.13: POST/GET /api/posts, blueprint posts_bp (ADR-004)
│   ├── application/
│   │   ├── auth/
│   │   │   ├── __init__.py
│   │   │   ├── login_use_case.py
...
│   │   └── posts/                    # IMPLEMENTADO — v0.13 (ADR-004)
│   │       ├── __init__.py
...
│   │   └── persistence/
│   │       ├── __init__.py
│   │       ├── models.py             # IMPLEMENTADO — v0.13 (+ modelo Post, ADR-004)
│   │       └── repositories/
│   │           ├── __init__.py       # IMPLEMENTADO — v0.6
│   │           ├── user_repository.py  # IMPLEMENTADO — v0.9 (SQLAlchemyUserRepository, + update())
│   │           └── post_repository.py  # IMPLEMENTADO — v0.13 (SQLAlchemyPostRepository, ADR-004)
│   ├── config.py
│   ├── extensions.py                  # v0.7: + callbacks de error JWT (ver sección 9)
│   └── __init__.py
├── tests/                             # IMPLEMENTADO — v0.13 (ver sección 15)
│   ├── __init__.py
│   ├── conftest.py
│   ├── test_auth.py
...
│   └── test_posts.py                  # IMPLEMENTADO — v0.13 (ADR-004)
├── migrations/                        # IMPLEMENTADO — v0.13 (ver sección 8, 4 migraciones)
├── run.py
