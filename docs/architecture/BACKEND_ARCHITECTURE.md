...
>
> Este documento no implementa, refactoriza ni modifica ningún código de `backend/`. Documenta lo que existe y propone, donde falta una decisión, el hueco explícito — nunca una arquitectura inventada.
...
| `requirements.txt` / `pyproject.toml` | No existen en el repositorio (`CLAUDE.md` §4, §10 lo confirma explícitamente) | Confirmado — no se encontró ninguno de los dos | Las versiones de esta tabla son un **snapshot observado del venv local**, no una lista de dependencias oficial ni reproducible. Fijar versiones y publicar el archivo de dependencias queda como `PENDIENTE DE APROBACIÓN` |
...
- **Credenciales hardcodeadas:** no hay usuarios reales, no hay tabla/modelo de usuario, no hay verificación contra una base de datos. Un único usuario fijo (`test@test.com` / `123456`) es "válido".
- **Sin hashing de contraseñas:** las contraseñas no se comparan mediante hash (ni bcrypt, ni werkzeug.security, ni ningún mecanismo) — es una comparación de texto plano en código fuente.
- **`JWT_SECRET_KEY` hardcodeado en el repositorio:** `config.py` define `JWT_SECRET_KEY = "super-secret-key"` como literal de código, versionado en git. Esto contradice la regla explícita de `HB-001` §19.1 ("nunca pegar ni loguear secretos reales") en espíritu, aunque técnicamente no es un secreto de un sistema en producción — es un placeholder de desarrollo, pero está en el repositorio sin mecanismo de sustitución por variable de entorno. Ver sección 16 (Seguridad).
...
- **`config.py`:** una única clase `Config` con un solo atributo, `JWT_SECRET_KEY`, como literal de texto en el código fuente.
- **Variables de entorno:** **no se usa ninguna.** No hay `os.environ`, no hay `python-dotenv`, no hay archivo `.env` ni `.env.example` en `backend/`. `run.py` fija `host="127.0.0.1"`, `port=5000` y `debug=True` como literales, no como configuración externa.
- **Secretos:** el único "secreto" del sistema (`JWT_SECRET_KEY`) vive hardcodeado en un archivo versionado en git — no hay separación entre secretos y código. `HB-001` §20 declara como regla innegociable que nunca se sube `.env` ni credenciales al repositorio; aquí no hay `.env` involucrado, pero el efecto práctico (un secreto en texto plano dentro del repositorio) es equivalente en riesgo. Se reporta como hallazgo de seguridad (sección 16), no se corrige aquí.
- **Configuración por ambiente:** **no existe.** Una sola clase `Config`, sin `DevelopmentConfig`/`ProductionConfig`/`TestingConfig`, sin selección de configuración según una variable de entorno (p. ej. `FLASK_ENV`).

`CLAUDE.md` §14 ya confirma este hueco: "Variables de entorno requeridas — solo la regla de no subir `.env`, sin lista oficial." Este documento no inventa esa lista.
...
| Área | Estado |
|---|---|
| Secretos | `JWT_SECRET_KEY` hardcodeado en `config.py`, versionado en git. Ningún mecanismo de variables de entorno o vault. |
| JWT | Emitido con `flask_jwt_extended` sobre una clave hardcodeada; sin política de expiración explícita; sin endpoints protegidos actualmente (`@jwt_required()` no se usa en ningún lugar del código); sin refresh tokens. |
...
| Datos sensibles | Contraseñas comparadas en texto plano dentro del código fuente (`domain/auth/auth_service.py`); sin hashing. |

`CLAUDE.md` §8 (fila Backend) ya anticipa este hueco: "Seguridad: sin documento oficial; aplicar buenas prácticas estándar... y señalar huecos, no llenarlos por inferencia silenciosa." Esta sección cumple ese mandato — señala, no resuelve.
...
│   └── api/
├── run.py
├── requirements.txt                   # PROPUESTA — no existe hoy
└── .env.example                       # PROPUESTA — no existe hoy
```
...
- Modelo de usuario real (hoy: credenciales hardcodeadas).
...
- Configuración por ambiente (`Development`/`Production`/`Testing`) y variables de entorno (`.env`).
- `requirements.txt` / `pyproject.toml` con versiones fijadas.
...
3. **Estrategia de hashing de contraseñas** (algoritmo, librería).
...
8. **Gestión de secretos** — cómo debe fijarse `JWT_SECRET_KEY` (variable de entorno, vault, etc.) fuera del código fuente.
...
14. **Archivo de dependencias oficial** (`requirements.txt` o `pyproject.toml`) y las versiones exactas a fijar.
