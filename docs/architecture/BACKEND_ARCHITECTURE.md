...
| PostgreSQL | Confirmado como motor elegido (`HB-001` portada; `CLAUDE.md` §4 "Base de Datos") | Driver y ORM ya integrados (ver filas arriba); **sigue sin haber una base PostgreSQL real conectada** — solo verificado localmente contra SQLite temporal durante el desarrollo de esta tarea, no contra Postgres | Parcialmente implementado. Ver sección 8 |
...
- **Acceso a PostgreSQL — v0.4, parcialmente implementado:** driver instalado (`psycopg[binary]==3.3.4`), `SQLALCHEMY_DATABASE_URI` leída desde `DATABASE_URL` en `config.py`, modelo `User` definido (`app/infrastructure/persistence/models.py`: `id` BIGINT autoincremental, `name`, `email` único+indexado, `password_hash`, `created_at`/`updated_at`). **Sigue sin haber una base PostgreSQL real conectada ni datos persistidos en producción/desarrollo compartido** — el flujo modelo→migración→`flask db upgrade` solo se verificó localmente contra SQLite temporal durante esta tarea (evidencia de que el código funciona, no un despliegue real).
